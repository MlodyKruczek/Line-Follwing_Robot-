...
//int orderArray [] = {forward, left, forward, forward, right, forward, pause};  // A -> S (A -> I -> J -> K -> L -> N -> S)
int orderArray[]  = {forward, right, forward, forward, right, forward, right, pause};  //D -> O (D -> E -> L -> N ->  S -> R -> Q -> O)
...
// < 900 atBlack  !!!!
  ...
  {  
         int order = chooseOrder(tabIterator++);
         executeOrder(order);
...
int chooseOrder(int ArrayIndex){
    return orderArray[ArrayIndex];
}
